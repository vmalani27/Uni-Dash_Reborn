...
def redis_enabled() -> bool:
    return bool(_redis_url()) and Redis is not None and AsyncRedis is not None
...
def get_async_redis_client() -> AsyncRedis:
    ...
    if _async_redis_client is None:
        _async_redis_client = AsyncRedis.from_url(_redis_url(), decode_responses=True)
...
def build_pipeline_payload(db: Session, uid: str) -> dict[str, Any]:
    ...
    ai_pending_count = (
        ...
        .filter(
            ...
            or_(
                GmailMessage.ai_status.is_(None),
                GmailMessage.ai_status == "pending",
                GmailMessage.ai_status == "processing",
            ),
        )
        .count()
    )
...
def publish_pipeline_event(db: Session, uid: str, source: str = "unknown") -> None:
    client = get_sync_redis_client()
    ...
    try:
        print(f"[EVENT] Publishing sync update uid={uid[:8]} source={source}")
